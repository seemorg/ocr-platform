...
export default function EditEmpirePage({
  empire,
}: {
  empire: {
    id: string;
  ...
  const [arabicName, setArabicName] = useState(empire.arabicName ?? "");
  const [englishName, setEnglishName] = useState(empire.englishName ?? "");
  const [transliteration, setTransliteration] = useState(
    empire.transliteration ?? "",
  );
  const [slug, setSlug] = useState(empire.slug);

  const router = useRouter();
  const { mutateAsync, isPending } = api.usulEmpire.update.useMutation({
    onSuccess: () => {
      toast.success("Empire updated");
      router.refresh();
    },
    onError: (error) => {
