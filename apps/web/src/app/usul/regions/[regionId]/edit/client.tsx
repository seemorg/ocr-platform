...
export default function EditRegionPage({
  ...
  const [arabicName, setArabicName] = useState(region.arabicName ?? "");
  const [englishName, setEnglishName] = useState(region.englishName ?? "");
  ...
  const { mutateAsync, isPending } = api.usulRegion.update.useMutation({
    onSuccess: () => {
